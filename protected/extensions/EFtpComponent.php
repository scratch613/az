...
class EFtpComponent extends CApplicationComponent
{
    ...
    public function connect()
    {
        if($this->_connection === null){
            ...
            if((!empty($this->username) OR !empty($this->password)) AND !$login_result)
                throw new CException('FTP Library Error: Login failed!');
	 ...
	 * @return	mixed
	 */
	public function dir($directory, $rec = false)
	{
        if($this->getActive()){
            $rawlist = @ftp_rawlist($this->_connection, $directory, $rec);
            return $rawlist;
        }
        else{
            throw new CDbException('EFtpComponent is inactive and cannot perform any FTP operations.');
    ...
    public function pasv($state)
	{
        if($this->getActive()){
            return ftp_pasv($this->_connection, $state);
        }
        else{
            throw new CDbException('EFtpComponent is inactive and cannot perform any FTP operations.');
        }
